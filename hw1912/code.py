# Whole flight video rec  |  pics at 5,1 ; 2,1 ; 4,5 ;  |  output temp
...
def screen_callback(data):
    ...
    cv_image = bridge.imgmsg_to_cv2(data , 'bgr8')
    cv2.putText(cv_image, str(temerature), (0, 230), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0)) #RGB HEX HSV BGR
    cv2.imwrite("img/{}.jpg".format(time())  ,   cv_image) 
    cv2.waitKey(1)
...
navigate_wait(x=5, y=1, z=2, frame_id='aruco_map')
screen_sub = rospy.Subscriber("main_camera/image_raw", Image, screen_callback)
screen_sub.unregister()


navigate_wait(x=2, y=1, z=2, frame_id='aruco_map')
screen_sub = rospy.Subscriber("main_camera/image_raw", Image, screen_callback)
screen_sub.unregister()

navigate_wait(x=4, y=5, z=2, frame_id='aruco_map')
screen_sub = rospy.Subscriber("main_camera/image_raw", Image, screen_callback)
screen_sub.unregister()
